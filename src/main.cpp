			...
			do {
				...
				case 2:
					...
					if (nodos[0].count(elem) || nodos[1].count(elem)) {
						cout << "Para asegurar la integridad de la aplicaci�n, no se permiten elementos repetidos, "
							 << "ni siquiera entre distintos mont�culos (aunque internamente los mont�culos s� lo permitan).\n\n";
						break;
					}
					nodos[mont][elem] = mf[mont].insert(elem);
					cout << "Se ha insertado el n�mero " << elem << "\n\n";
					break;
				case 3:
					mf[mont] = mf[mont].unionFib(mf[(mont + 1) % 2]);
					nodos[mont].insert(nodos[(mont + 1) % 2].begin(), nodos[(mont + 1) % 2].end());
					nodos[(mont + 1) % 2].clear();
					cout << "Se han transferido todos los elementos de Mont�culo_" << (mont + 1) % 2 + 1 << " a Mont�culo_" << mont + 1 << ".\n\n";
					break;
				case 4:
					nodos[mont].erase(mf[mont].top());
					cout << "Se ha eliminado el n�mero " << mf[mont].borraTop() << ".\n\n";
					break;
				case 5:
					cout << "Elemento a decrecer: ";
					cin >> elem;
					if (!nodos[mont].count(elem)) {
						cout << "Elemento no existente en Mont�culo_ " << mont + 1 << ".\n\n";
						break;
					}
					cout << "Nuevo valor: ";
					cin >> nuevo;
					...
					cout << "Se ha cambiado el n�mero " << elem << " por " << nuevo << ".\n\n";
					break;
				case 6:
					cout << "Mont�culo_ " << mont + 1 << " en forma de lista de nodos con las listas de sus hijos:\n"
						 << mf[mont].show() << "\n\n";
					break;
				case 7:
					med = menuMed() -1;
					if (med < 0) break;
					cout << "Midiendo operaci�n, puede tardar unos segundos...\n";
...
int menuMf() {
	cout << "Elija un mont�culo a utilizar (ambos son mont�culos de m�nimos para n�meros enteros):\n"
		 << "	1.- Mont�culo_1\n"
		 << "	2.- Mont�culo_2\n"
		 << "	0.- Salir\n";
	int op;
...
int menuMed() {
	cout << "Operaciones:\n"
		 ...
		 << "	0.- Volver\n";
	int op;
	cout << "Elija una operaci�n a medir:";
	cin >> op;
	while (op < 0 || op > 5) {
		cout << "Opci�n no v�lida.\nElija una operaci�n:";
		cin >> op;
